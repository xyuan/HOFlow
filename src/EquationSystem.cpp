...
void EquationSystem::initial_work() {
    std::vector<Algorithm *>::iterator ii;
    for( ii=copyStateAlg_.begin(); ii!=copyStateAlg_.end(); ++ii )
        (*ii)->execute();
}

void EquationSystem::report_invalid_supp_alg_names() {
    ...
    if (!noInvalidNamesFound) {
        ...
        std::string msg = "Invalid supplemental algorithms name(s) for " + eqnTypeName_ + ". See log for details.";
        throw std::runtime_error(msg);
    }
}

void EquationSystem::report_built_supp_alg_names() {
...
void EquationSystem::update_iteration_statistics(const int & iters) {
...
bool EquationSystem::bc_data_specified(const UserData &userData, std::string &name) {
    bool isSpecified = false;
    std::map<std::string, bool>::const_iterator iter = userData.bcDataSpecifiedMap_.find(name);
