  ...
  double *error)
{
  ...
  if ( lerr )
    HOFlowEnv::self().hoflowOutput() << "sorry, negative TetSCS volume.." << std::endl;
}
  ...
  double *error)
{
  ...
  for ( int n=0; n<nelem; n++ ) {
    ...
    for ( int k=0; k<npf; k++ ) {
      ...
      if ( lerr )
        HOFlowEnv::self().hoflowOutput() << "sorry, issue with face_grad_op.." << std::endl;
  ...
  double *error)
{
  ...
  for ( int n=0; n<nelem; n++ ) {
    ...
    for ( int k=0; k<npf; k++ ) {
      ...
      if ( lerr )
        HOFlowEnv::self().hoflowOutput() << "sorry, issue with shifted_face_grad_op.." << std::endl;
    }
  }
}
