...
void TimeIntegrator::integrate_realm() {
...
void TimeIntegrator::provide_mean_norm() {
...
bool TimeIntegrator::simulation_proceeds() {
    bool proceed = false;
    if ( terminateBasedOnTime_ ) {
...
double TimeIntegrator::get_time_step(const HOFlowState &theState) const {
...
double TimeIntegrator::get_current_time() const {
    return currentTime_;
}

void TimeIntegrator::compute_gamma() {
    // defaults
    gamma1_ = 1.0;
    gamma2_ = -1.0;
    gamma3_ = 0.0;
...
double TimeIntegrator::get_gamma1() const {
    return gamma1_;
}

double TimeIntegrator::get_gamma2() const {
    return gamma2_;
}

double TimeIntegrator::get_gamma3() const {
    return gamma3_;
}

int TimeIntegrator::get_time_step_count() const {
    return timeStepCount_;
}

double TimeIntegrator::get_time_step_from_file() {
    return timeStepFromFile_;
}

bool TimeIntegrator::get_is_fixed_time_step() {
    return ! adaptiveTimeStep_ ;
}

bool TimeIntegrator::get_is_terminate_based_on_time() {
    return terminateBasedOnTime_;
}

double TimeIntegrator::get_total_sim_time() {
    if ( terminateBasedOnTime_) 
...
int TimeIntegrator::get_max_time_step_count() {
    if ( ! terminateBasedOnTime_) 
