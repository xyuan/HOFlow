  ...
  if (y_output)
  {
    ...
    if ( outputCompressionShuffle_ )
      if ( outputCompressionLevel_ == 0 ) 
        HOFlowEnv::self().hoflowOutputP0() << "OutputInfo::load() Output Warning: One should not shuffle if one is not compressing" << std::endl;
    
    // serialize io...
    {
      get_if_present(y_output, "serialized_io_group_size", serializedIOGroupSize_, serializedIOGroupSize_);
      if (serializedIOGroupSize_) {
        HOFlowEnv::self().hoflowOutputP0() << "Info: found non-zero serialized_io_group_size in input file= " << serializedIOGroupSize_ << std::endl;
      }
    }
  ...
  {    
    ...
    if ( restartCompressionShuffle_ )
      if ( restartCompressionLevel_ == 0 )  
        HOFlowEnv::self().hoflowOutputP0() << "OutputInfo::load() Restart Warning: One should not shuffle if one is not compressing" << std::endl;
    ...
    if (y_vars) {
      HOFlowEnv::self().hoflowOutputP0() << "Restart variable specification has been deprecated" << std::endl;
    }
  }
}
