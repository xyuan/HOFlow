    ...
    if ( userData.tempSpec_ ||  FUNCTION_UD == theDataType ) {
        ...
        if ( itd == solverAlgDriver_->solverDirichAlgMap_.end() ) {
            ...
            if (realm_.solutionOptions_->useNaluBC_) {
                ...
                DirichletBC * theAlg = new DirichletBC(realm_, this, part, &tempNp1, theBcNodalField, 0, 1);
                solverAlgDriver_->solverDirichAlgMap_[algType] = theAlg;
            }
            else {
                // Use CFX style Dirichlet BC, fix the IP value to the specified value
                AssembleScalarDirichletBCSolverAlgorithm * theAlg = new AssembleScalarDirichletBCSolverAlgorithm(realm_, 
                                                                                                                 part, 
                                                                                                                 this, 
                                                                                                                 &tempNp1, 
                                                                                                                 theBcField, 
