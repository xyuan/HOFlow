  ...
  std::map<std::string, SolverAlgorithm*>& solverAlgs)
{
  ...
  if (createNewAlg) {
    ...
    HOFlowEnv::self().hoflowOutputP0() << "Created the following interior elem alg: " << algName << std::endl;
  ...
  const std::string bcName)
{
  ...
  if (createNewAlg) {
    ...
    HOFlowEnv::self().hoflowOutputP0() << "Created the following bc face/elem alg: " << algName << std::endl;
    solverAlgs.insert({algName, theSolverAlg});
  }
  else {
    auto& partVec = itc->second->partVec_;
  ...
  const std::string bcName)
{
  ...
  if (createNewAlg) {
    ...
    HOFlowEnv::self().hoflowOutputP0() << "Created the following bc face alg: " << algName << std::endl;
    solverAlgs.insert({algName, theSolverAlg});
  }
  else {
    auto& partVec = itc->second->partVec_;
