...
inline DeviceTeamPolicy get_team_policy(const size_t sz, const size_t bytes_per_team, const size_t bytes_per_thread) {
...
inline SharedMemView<int*> get_int_shmem_view_1D(const TeamHandleType& team, size_t len) {
    return Kokkos::subview(SharedMemView<int**>(team.team_shmem(), team.team_size(), len), team.team_rank(), Kokkos::ALL());
}

inline SharedMemView<stk::mesh::Entity*> get_entity_shmem_view_1D(const TeamHandleType& team, size_t len) {
    return Kokkos::subview(SharedMemView<stk::mesh::Entity**>(team.team_shmem(), team.team_size(), len), team.team_rank(), Kokkos::ALL());
}

template<typename T> SharedMemView<T*> get_shmem_view_1D(const TeamHandleType& team, size_t len) {
    return Kokkos::subview(SharedMemView<T**>(team.team_shmem(), team.team_size(), len), team.team_rank(), Kokkos::ALL());
}

template<typename T> SharedMemView<T**> get_shmem_view_2D(const TeamHandleType& team, size_t len1, size_t len2) {
    return Kokkos::subview(SharedMemView<T***>(team.team_shmem(), team.team_size(), len1, len2), team.team_rank(), Kokkos::ALL(), Kokkos::ALL());
}

template<typename T> SharedMemView<T***> get_shmem_view_3D(const TeamHandleType& team, size_t len1, size_t len2, size_t len3) {
...
template<typename SizeType, class Function> void kokkos_parallel_for(const std::string& debuggingName, SizeType n, Function loop_body) {
